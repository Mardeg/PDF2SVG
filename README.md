...
It is **not** intended for standard text-selectable digital layout documents (such as vectorized PDFs containing real type font geometry layers… why did I whisper that?). Also it doesn't currently do batch processing or include detection for non-PNG bitmap images inside PDFs… I think. If it does other image formats than that wasn't intentional.
